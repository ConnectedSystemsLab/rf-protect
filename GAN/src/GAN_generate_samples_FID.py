...
def generate_sample(g_model, batches_done):
    ...
    gen_labels = np.array([num for num in range(n_row) for num2 in range(600)])
    ...
    gen_imgs = gen_imgs.detach().numpy()
    for j in range(3000):
        scipy.io.savemat("./check_tmp/data_fake/{}.mat".format(j), {'M': gen_imgs[j]})
    # fig = plt.figure(figsize=(17, 17))
    # for i_s in range(1, 26):
...
def main(args):
    ...
    #  Pre training
    # ---------------------
    generator_stat = torch.load('./Model_download/g_epoch_3230.pt')['model_state_dict']
...
if __name__ == "__main__":
    ...
    DATA_DIR_TRN = '../../DATA/nsdi_DATA_c_50_10_50'.format(ARGS.time_length, ARGS.trace_sample_interval)
