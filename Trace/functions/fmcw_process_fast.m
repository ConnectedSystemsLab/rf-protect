...
fft_len=951;
fmcw_frames=zeros(1200,fft_len,181);
num_antennas=8;
...
d1 = floor(length(c0_f)/num_antennas);
c0_f = c0_f(1:d1*num_antennas,:);
avg_frames = reshape(c0_f,  num_antennas, [],fft_len);
avg_frames = permute(avg_frames, [2,1,3]);
d_tmp = size(avg_frames);
d2 = d_tmp(1);
d2 = floor(d2/avg_fact);
avg_frames = avg_frames(1:d2*avg_fact,:,:);
avg_frames = reshape(avg_frames, avg_fact,d2, num_antennas, fft_len);
...
cosp=cos(phi).';
aoa_vec_array = zeros(fft_len, 181, num_antennas);
for i1 = 1:fft_len
